...
The shared state helper stores durable learning progress in SQLite
...
The active repository adapter owns manifest parsing and projection. Read its
base command from `.learning-mentor.toml` and use its `validate` and
`state-projection` operations. Do not reproduce its ecosystem-specific
invocation or parse its manifest in the shared skill.
...
Use the [bundled helper](../scripts/learning_state.py); never open or mutate its
SQLite file with ad hoc SQL. Run it through the state command declared in
`.learning-mentor.toml` from the repository root. The Python interpreter
launches mentor infrastructure only; all course execution commands remain the
learning path's responsibility.
...
For the initial Python-hosted release, first inspect the interface:

```text
python3 .agents/skills/guided-learning/scripts/learning_state.py --help
python3 .agents/skills/guided-learning/scripts/learning_state.py <command> --help
```
